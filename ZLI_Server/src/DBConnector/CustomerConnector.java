	...
	public boolean createOrder(Order order) {
		try {
			...
			stmt = GeneralConnector.conn
					.prepareStatement("INSERT INTO `zliproject`.`orders` (`orderID`, `userID`, `greeting`,"
							+ " `isSelfMade`, `orderDetails`, `hasDelivery`, `address`, `deliveryDate`, `branch`, `paymentDetails`, `price`, `status`,orderDate) "
							+ "VALUES ('?', '?', '?', '?', '?', '?', '?', '?', '?', '?', '?', '?','?');");
